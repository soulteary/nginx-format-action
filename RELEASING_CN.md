...
当标签与 `package.json` 中的版本不一致，或标签提交不属于 `main` 时，工作流会在发布前停止。只有已有 Release 记录的提交与完整版本标签一致时，重新运行才会继续；如果记录缺失或提交不一致，工作流会在移动兼容标签前失败。匹配的 GitHub Release 不会被覆盖，主版本和次版本引用会再次更新。
